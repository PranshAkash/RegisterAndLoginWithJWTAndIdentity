...
namespace Data
{
    public class DapperRepository : IDapperRepository, IDisposable
    {
        ...
        public DapperRepository(IConfiguration config, IConnectionHelper connectionString)
        {
            _config = config;
        ...
        public async Task<int> ExecuteAsync(string sp, object param = null, CommandType commandType = CommandType.StoredProcedure, string connectionString = "")
        {
            int i = -1;
            try
            {
        ...
        public async Task<T> GetAsync<T>(string sp, object parms = null, CommandType commandType = CommandType.Text, string connectionString = "")
        {
            T result;
            try
            {
        ...
        public async Task<IEnumerable<T>> GetAllAsync<T>(string sp, object parms = null, CommandType commandType = CommandType.StoredProcedure, string connectionString = "")
        {
            try
            {
                if (string.IsNullOrEmpty(connectionString))
